...
class MultiHeadAutoBackend(nn.Module):
    ...
    def __init__(self,
        ...
        if nn_module:
            ...
            if hasattr(model, 'kpt_shape'):
                kpt_shape = model.kpt_shape  # pose-only
            stride = max(int(model.stride.max()), 32)  # model stride
            names = model.module.names[head] if hasattr(model, 'module') else model.names[head]  # get class names
            model.half() if fp16 else model.float()
            self.model = model  # explicitly assign for to(), cpu(), cuda(), half()
            pt = True
        elif pt:  # PyTorch
        ...
        if 'names' not in locals():  # names missing
            names = self._apply_default_class_names(data)
        names = [check_class_names(n) for n in names]

        self.__dict__.update(locals())  # assign all variables to self

    def forward(self, im, augment=False, visualize=False, heads=None):
        """
        Runs inference on the YOLOv8 MultiBackend model.
        ...
        if self.pt or self.nn_module:  # PyTorch
            y = self.model(im, augment=augment, visualize=visualize, heads=heads) if augment or visualize else self.model(im, heads=heads)
        elif self.jit:  # TorchScript
            y = self.model(im)
        elif self.dnn:  # ONNX OpenCV DNN
            im = im.cpu().numpy()  # torch to numpy
    ...
    def warmup(self, imgsz=(1, 3, 640, 640)):
        ...
        if any(warmup_types) and (self.device.type != 'cpu' or self.triton):
            im = torch.empty(*imgsz, dtype=torch.half if self.fp16 else torch.float, device=self.device)  # input
            for _ in range(2 if self.jit else 1):  #
                self.forward(im, heads=torch.zeros(1, dtype=torch.long, device=self.device))  # warmup
